...
def analyse_bw(network_state_file, outpath):
  ...
  adv_bw_nodes, guard_fraction_bw, exit_fraction_bw = {}, {}, {}
  obs_bw_nodes, obs_fraction_guard_bw, obs_fraction_exit_bw = {}, {}, {}

  measured_bw_nodes, measured_guard_fraction_bw, measured_exit_fraction_bw = {}, {}, {}
  ...
  #probability
  guard_fraction, exit_fraction = compute_weighted_bw_fraction(guards, exits, guard_prob,\
...
#def set_weights_for_multipath(nodes, weights, measured_bw, obs_bw, descriptors):

  #measured_bw_list_r = measured_bw.items()
  #measured_bw_list_r.sort(lambda x: x[1][0], reverse = True)
  #measured_bw_list_i = measured_bw_list_r[:] #copy list
  #measured_bw_list_i.reverse()
...
def plot_bw_comparaison(adv_bw, measured_bw, obs_bw, weights):
  ...
  obs_bw_list = obs_bw.items()
  obs_bw_list.sort(key=lambda x: x[1], reverse=True)
  y2 = [bw/1000.0 for (relay, bw) in obs_bw_list]
  y3 = [adv_bw[relay]/1000.0 for (relay, bw) in measured_bw_list]
  if len(weights) > 0:
    y4 = [weights[relay] for (relay, bw) in measured_bw_list]
  adv_bw_list = adv_bw.items()
  adv_bw_list.sort(key=lambda x: x[1], reverse=True)
  y5 = [bw/1000.0 for (relay, bw) in adv_bw_list]
  x_unmeasured = []
  y_unmeasured = []
  i = 0
  for (key, (bw, is_unmeasured)) in measured_bw_list:
    ...
    i+=1
  pdb.set_trace()
  matplotlib.pyplot.scatter(x, y2, color="red", s=1)
  ...
  matplotlib.pyplot.scatter(x, y5, color="black", s=1)
...
def compute_weighted_bw_fraction(guards, exits, guard_prob, middle_prob,\
    exit_prob):
  guard_fraction = {}
  exit_fraction = {}
  for relay in guards:
  ...
  for relay in exits:
     exit_fraction[relay] = exit_prob[relay]/(exit_prob[relay]+guard_prob[relay]+\
         middle_prob[relay])
  return guard_fraction, exit_fraction
