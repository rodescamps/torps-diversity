...
def plot_cdf(lines, line_labels, xlabel, title, location, out_pathname,
    ...
    if (line_labels != None):
        ...
        for data_points, line_label in zip(lines, line_labels):
            ...
            i += 1
        matplotlib.pyplot.legend(loc='best', fontsize = fontsize)
    else:
        x, y = getcdf(lines)
...
def first_compromise_times(start_time, end_time, stats_list):
    ...
    return (guard_times, exit_times, guard_and_exit_times)
...
def compromised_set_plot_times(start_times, end_times, compromise_stats,
    ...
    for start_time, end_time, stats_list in zip(start_times, end_times,
        compromise_stats):    
        guard_times, exit_times, guard_and_exit_times = first_compromise_times(start_time, end_time,
            stats_list)
...
def read_analysis_files(pathnames):
    ...
    return (start_time, end_time, compromise_stats)
...
def compromised_set_plot(pathnames_list, line_labels, out_dir, out_name,
    ...
    for i, pathnames in enumerate(pathnames_list):
        if (out_name == None):
            filename = os.path.basename(pathnames[0])
    ...
    compromised_set_plot_rates(compromise_stats, line_labels, out_dir,
        out_name, figsize, fontsize, legend_locs = rate_legend_locs)
...
def compromised_top_relays_plot(pathnames, out_dir):
    ...
    # aggregate the stats        
    start_time, end_time, compromise_stats = read_analysis_files(pathnames)
...
if __name__ == '__main__':
    usage = 'Usage: pathsim_plot.py [plot type] [in_dir] [out_dir] [out_name]: \nTakes \
all files in in_dir, plots their contents according to type, and outputs the results to \
out_dir. Plot type is one of "set" or "top". out_name is optional; if omitted, it is \
assumed that input filenames are of form x.y.z, and output files will use x for out_name.'
    ...
    if (plot_type == 'set'):
        ...
        line_labels = ["Tor stable release", "Tor with water filling"]
        #line_labels = None
        #line_labels = ["Tor 2.4.x", "Tor with water filling"]
        #pdb.set_trace()
